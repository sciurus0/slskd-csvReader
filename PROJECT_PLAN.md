...
- [x] **Search query string construction (featured artists / multi-artist rows)** — implemented on `feature/query-normalization-fallbacks` (bounded literal-then-softened query variants; weak `duration_ms` tie-break among ranked candidates only).
